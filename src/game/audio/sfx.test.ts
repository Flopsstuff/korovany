...
describe('sfx registry', () => {
  ...
  it.each(SFX_NAMES.filter((n) => n !== 'forestAmbience'))(
  ...
  it('forestAmbience renders to non-empty, in-range PCM (with longer timeout)', () => {
    const pcm = renderSfx('forestAmbience', 44100)
    expect(pcm.length).toBeGreaterThan(0)
